...
# -*- coding: utf-8 -*-
"""
FishAudio TTS API 客户端（基于FishSpeech开源模型）
"""
import os
import sys
import json
...
# API Key 优先级: 参数 > 环境变量 FISHAUDIO_API_KEY
...
def make_request(method, path, data=None, files=None):
    ...
    if not API_KEY:
        return {"code": -1, "msg": "未配置API Key，请设置环境变量 FISHAUDIO_API_KEY 或传入 --api-key 参数，Key可前往 https://fishaudio.cc 获取"}
...
def create_tts(text, audio_id, style="1", genre=None, speed=None, emotion_path=None, output=None):
    ...
    if result.get('code') != 0:
        print(f"❌ FishAudio 合成失败: {result.get('msg')}")
        return None
    
    task_id = result['data']['taskId']
    print(f"⏳ FishAudio TTS任务已创建: {task_id}")
        ...
        if query_result.get('code') == 0:
            ...
            if status == 2 or status == 3:
                ...
                if voice_url:
                    # 确定输出文件名
                    if not output:
                        output = f"fishaudio_tts_{task_id[:8]}.wav"
                    # 下载音频
                    try:
                        ...
                        print(f"✅ FishAudio 合成完成！已保存到: {os.path.abspath(output)}")
...
def delete_model(audio_id):
    ...
    if result.get('code') == 0:
        print(f"✅ FishAudio 模型已删除: {audio_id}")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="FishAudio TTS 语音合成工具（基于FishSpeech模型），API Key前往 https://fishaudio.cc 获取")
    parser.add_argument('--api-key', help='FishAudio API Key')
    subparsers = parser.add_subparsers(dest='command', help='可用命令')
    
    # 上传模型
    p_upload = subparsers.add_parser('upload', help='上传参考音频创建声音克隆模型')
    p_upload.add_argument('--file', required=True, help='参考音频文件路径 (支持mp3/wav/m4a，建议使用20秒左右清晰人声)')
    ...
    p_tts.add_argument('--style', default='1', help='模型版本: 1=基础模型, 2=专业模型(支持情感控制), 3=多语言模型 (默认: 1)')
    ...
    p_tts.add_argument('--output', '-o', help='本地输出wav文件路径 (脚本参数, 默认: fishaudio_tts_<taskid>.wav, API本身无此参数)')
