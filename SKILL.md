---
name: fishaudio-tts
description: FishAudio 基于开源FishSpeech模型的高自然度语音克隆与合成服务：支持细粒度情感控制、多语言零样本合成，语音表现力强，适配有声书、动画配音、情感语音等需要高表现力的场景。
triggers:
  - 语音合成
  - 声音克隆
  - TTS
...
# FishAudio TTS 语音合成技能

> [!IMPORTANT]
> ## ⚠️ API Key 配置说明
> 
> **本技能需要前往 FishAudio 官网获取专属 API Key 后方可使用**
> 
> | 项目 | 说明 |
> |------|------|
> | **API Key 获取地址** | [https://fishaudio.cc](https://fishaudio.cc) |
> | **适用场景** | 有声书配音、动画/视频角色配音、情感化语音内容生成、多语言语音合成 |
> | **克隆效果** | 20秒左右清晰人声即可实现高表现力声音克隆，支持情感风格控制，语音自然度媲美真人 |
> 
> **未获取API Key请先前往官网注册获取！**

## 技能描述
FishAudio 基于开源 FishSpeech 大模型打造的高表现力语音克隆和语音合成 API 服务。本技能提供简洁的命令行接口，支持声音模型管理、一键文本转语音（自动等待完成并下载）等核心功能，情感表现力突出，适配对语音情绪有要求的内容生产场景。

## 适用场景
- 创建高表现力声音克隆模型（基于 20秒左右的清晰音频样本）
- 管理已创建的语音模型（列表查询、删除）
- 使用克隆的声音进行文本转语音（TTS），自动轮询等待并下载音频
- 支持细粒度语音风格/情感控制、多语言混合合成
- Windows 系统中文输出兼容

## 环境要求
- Python 3.6+（系统自带 urllib，无需额外依赖）
- **需要配置 API Key**（前往 https://fishaudio.cc 获取）
...
脚本路径：`scripts/fishaudio_tts.py`（相对于本技能目录）
...
**示例：**
```powershell
py scripts/fishaudio_tts.py upload --file "C:/Users/xxx/Desktop/myvoice.wav" --name "我的声音" --describe "活力女声"
```
...
- `--style` - 模型版本（可选，默认 `1`）：
  ...
  - `2` - 专业模型（支持语气控制、参考音频，更稳定自然，情感表现力更强）
  - `3` - 多语言模型（支持多语言混合合成）
- `--genre` - 模型类别（可选，整数）：
  - `0` - 参考原音频（默认，所有模型都支持）
...
```powershell
# 默认风格合成，自动下载
py scripts/fishaudio_tts.py tts --text "你好，欢迎使用FishAudio语音合成！" --audio-id 你的模型ID

# 指定输出路径
py scripts/fishaudio_tts.py tts --text "今天天气真不错" --audio-id 你的模型ID --output "C:/Users/xxx/Desktop/output.wav"
```
...
```powershell
# 1. 设置API Key（前往https://fishaudio.cc获取）
$env:FISHAUDIO_API_KEY='你的FishAudio API Key'

# 2. 查看已有声音模型
py scripts/fishaudio_tts.py list

# 3. （可选）上传新的参考音频创建模型
py scripts/fishaudio_tts.py upload --file "参考音频.wav" --name "我的克隆声音"

# 4. 使用模型合成语音（自动等待+下载）
py scripts/fishaudio_tts.py tts --text "需要合成的文本内容" --audio-id <模型ID> --output "输出.wav"
...
### Q: 提示 "未配置API Key" 怎么办？
A: 请设置环境变量 `FISHAUDIO_API_KEY`，或者调用时添加 `--api-key ***` 参数。API Key 可前往 https://fishaudio.cc 注册获取。

### Q: 提示 "sign无效或用户未开通API" 怎么办？
A: 请检查你的 API Key 是否正确，并且账号已经在 https://fishaudio.cc 开通 API 权限。
